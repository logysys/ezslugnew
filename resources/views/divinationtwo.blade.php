...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    async function go() {
      ...
      document.getElementById('agentTag').innerText = '—';
      ...
      try {
        ...
        let errorMsg = err.message?.substring(0, 200) || '連線異常';
        if (errorMsg.includes('Unterminated string') || errorMsg.includes('JSON')) {
          errorMsg = 'AI 回傳格式異常，請稍後再試。 (JSON 解析錯誤)';
        }
        setMsg(`⚠️ ${currentAgent.toUpperCase()} 占卜暫時中斷，請稍後再試。`, '', '', '');
        showErr(`${currentAgent.toUpperCase()} 錯誤：${errorMsg}`);
        // Still update with default pricing
        const defaultPricing = { input: 0.15, output: 0.60, label: '—' };
        updateCost(currentAgent, 150, 300, defaultPricing);
      } finally {
        btn.disabled = false;
      }
    }
