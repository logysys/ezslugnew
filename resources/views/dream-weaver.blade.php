...
<script>
    ...
    function updateStatsUI() {
        document.getElementById('cacheCount').innerText = storyCacheCount || 0;
        ...
        document.getElementById('detailCostSpan').innerHTML = `📊 Kimi: ¥${totalKimiCost.toFixed(6)} | Perplexity: ¥${totalPerplexityCost.toFixed(6)} | 缓存: ${storyCacheCount || 0}`;
    }

    let storyCacheCount = 0;
    ...
    async function fetchOrGenerate(word, style) {
        if (currentMode === 'offline') {
            ...
            return { content: result.content, tokens: null, mode: 'offline' };
        }
        
        toastMessage(`${currentMode === 'kimi' ? '🌙 Kimi K3' : '🔮 Perplexity Sonar Pro'} 编织梦境「${word}」...`, 'info');
        ...
        try {
            ...
            return { content: result.content, tokens: result.tokens, mode: result.mode };
        } catch (err) {
            console.error(err);
            if (currentMode === 'perplexity') {
            ...
            return { content: fallback.content, tokens: null, fallback: true, mode: 'offline' };
        }
    }
    ...
    async function generateDream() {
        ...
        try {
            ...
            displayStoryOnUI(word, storyRes.content, currentStyle, false, storyRes.mode || currentMode);
            historyList.unshift({ word, content: storyRes.content, style: currentStyle, mode: storyRes.mode || currentMode, timestamp: Date.now() });
    ...
    function displayStoryOnUI(word, content, style, fromCacheHint, modeUsed) {
        ...
        const modeLabel = modeUsed === 'kimi' ? '🌙 Kimi K3' : (modeUsed === 'perplexity' ? '🔮 Perplexity Sonar Pro' : '📖 离线');
        container.innerHTML = `
    ...
    function renderHistory() {
        ...
        if (!historyList.length) { histDiv.innerHTML = '<div class="empty-message">🌸 暂无梦境记录</div>'; return; }
    ...
    function renderFav() {
        ...
        if (!favoriteList.length) { favDiv.innerHTML = '<div class="empty-message">💖 收藏梦境显示于此</div>'; return; }
    ...
    async function fetchCacheStats() {
        try {
            const response = await fetch('/dream/cache-stats', {
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                }
            });
            ...
            if (result.count !== undefined) {
                storyCacheCount = result.count;
                document.getElementById('cacheCount').innerText = result.count;
            }
        } catch(e) {
            // Silent fail
        }
    ...
    function init() {
        ...
        document.querySelectorAll('.mode-pill').forEach(btn => { 
            btn.addEventListener('click', () => { 
                ...
                persistAll(); 
                toastMessage(`模式切换至 ${currentMode === 'kimi' ? '🌙 Kimi K3 (需激活标签)' : currentMode === 'perplexity' ? '🔮 Perplexity Sonar Pro' : '📖 离线诗梦'}`, 'info'); 
            }); 
        });
