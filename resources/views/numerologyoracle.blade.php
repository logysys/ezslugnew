  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      ...
      font-family: 'Inter', system-ui, -apple-system, sans-serif; padding: 1.5rem;
    }
    .container {
      max-width: 880px; width: 100%; background: rgba(12,9,28,0.85);
      backdrop-filter: blur(16px); border-radius: 2.5rem;
      border: 1px solid rgba(180,140,255,0.45); padding: 2rem;
      box-shadow: 0 25px 45px -15px black;
    }
    h1 {
      font-size: 1.9rem;
      background: linear-gradient(120deg, #f2eaff, #cdadff, #b47eff);
      background-clip: text; -webkit-background-clip: text; color: transparent; text-align: center;
    }
    .sub { text-align: center; color: #bbaee6; margin: 0.5rem 0 1.5rem; font-size: 0.8rem; }

    /* LANGUAGE SELECTOR */
    .lang-section {
      background: rgba(20,12,45,0.7); border: 1px solid rgba(140,100,255,0.3);
      border-radius: 1.4rem; padding: 0.9rem 1.1rem; margin-bottom: 1.4rem;
    }
    .lang-section-label {
      font-size: 0.68rem; text-transform: uppercase; letter-spacing: 2px;
      color: #a092d8; margin-bottom: 0.65rem; display: flex; align-items: center; gap: 8px;
      flex-wrap: wrap;
    }
    .lang-section-label span { color: #c49eff; font-weight: 600; }
    .lang-pills { display: flex; flex-wrap: wrap; gap: 7px; }
    .lang-pill {
      background: rgba(80,55,140,0.35); border: 1px solid #5a48a0;
      padding: 0.3rem 0.75rem; border-radius: 30px; color: #cdbcff;
      cursor: pointer; font-size: 0.78rem; transition: 0.18s; user-select: none;
    }
    .lang-pill:hover { background: rgba(120,85,200,0.5); border-color: #9a7eff; }
    .lang-pill.active {
      background: linear-gradient(95deg, #5a3dba, #9a6fff);
      border-color: #c49eff; color: white; box-shadow: 0 0 8px rgba(160,110,255,0.5);
    }
    ...
    .agent-btn { background:#1a1538; border:1px solid #5a48a0; padding:0.5rem 1rem; border-radius:40px; color:#cdbcff; cursor:pointer; font-weight:500; font-size:0.85rem; transition:0.2s; }
    ...
    .ask-btn { background:linear-gradient(95deg,#755aea,#a47eff); border:none; padding:0 1.8rem; border-radius:60px; font-weight:bold; color:white; cursor:pointer; font-size:1rem; transition:transform 0.1s; white-space:nowrap; }
    ...
    .extra { background:rgba(210,180,255,0.08); padding:0.8rem; border-radius:1rem; font-style:italic; font-size:0.85rem; color:#ddd0ff; margin-bottom:1rem; }
    ...
    /* LANGUAGE BADGE on result */
    .lang-badge {
      display:inline-flex; align-items:center; gap:5px;
      background:rgba(100,70,200,0.25); border:1px solid rgba(160,120,255,0.4);
      border-radius:20px; padding:0.2rem 0.65rem; font-size:0.68rem; color:#c4adff;
    ...
    .error-popup { position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:rgba(20,10,40,0.98); border:1px solid #ff6b6b; border-radius:1.5rem; padding:2rem; max-width:420px; width:90%; z-index:1000; text-align:center; box-shadow:0 20px 60px rgba(0,0,0,0.8); display:none; }
...
<div class="container">
  ...
  <div class="lang-section">
    <div class="lang-section-label">🌐 Reading language &nbsp;·&nbsp; Oracle will respond in <span id="selectedLangLabel">English</span></div>
  ...
  <div id="resultArea">
    <div class="card">
      <div id="resultLangBadge" style="display:none;" class="lang-badge"></div>
...
<script>
  ...
  const API_BASE = '/api/numerologyall';
  let currentAgent = 'perplexity';
  let selectedLang = { code: 'en', label: '🇺🇸 English' };
  ...
  const resultLangBadge = document.getElementById('resultLangBadge');
  ...
  Object.entries(LANGUAGES).forEach(([code, lang], i) => {
    ...
    pill.onclick = () => {
      ...
      selectedLang = { code, label: `${lang.flag} ${lang.name}` };
      selectedLangLabel.textContent = lang.name;
    };
  ...
  function formatUSD(cost) {
    ...
    if (numCost < 0.000001) return `$${numCost.toExponential(4)}`;
    return `$${numCost.toFixed(5)}`;
  }

  // ========== FIXED updateCostUI function ==========
  function updateCostUI(agent, inp, out, inputCost, outputCost, totalCost) {
    // Ensure all values are numbers
    const safeInp = Number(inp) || 0;
    const safeOut = Number(out) || 0;
    const safeInputCost = Number(inputCost) || 0;
    const safeOutputCost = Number(outputCost) || 0;
    const safeTotalCost = Number(totalCost) || 0;
    
    inputTokensSpan.innerText = safeInp.toLocaleString();
    outputTokensSpan.innerText = safeOut.toLocaleString();
    inputCostSpan.innerText = formatUSD(safeInputCost);
    outputCostSpan.innerText = formatUSD(safeOutputCost);
    totalCostSpan.innerText = formatUSD(safeTotalCost);
    agentNameDisplay.innerText = String(agent).toUpperCase();
  }

  // ========== API CALLS TO LARAVEL BACKEND ==========
  
  async function getNumerology(number, agent, language) {
    const response = await fetch(`${API_BASE}/predict`, {
      ...
      credentials: 'include',
      body: JSON.stringify({ number, agent, language })
    });
  ...
  async function loadStats() {
    try {
      ...
      if (data.success && data.data) {
        const totalSearches = Number(data.data.total_searches) || 0;
        const totalCost = Number(data.data.total_cost) || 0;
        statsBadge.innerHTML = `📊 ${totalSearches} searches | 💰 ${formatUSD(totalCost)}`;
      } else {
  ...
  function displayHistory(historyItems) {
    ...
    historyList.innerHTML = historyItems.map(item => {
      ...
      const watchOut = String(item.watch_out || '');
      return `
        <div class="history-item">
          <div style="display:flex;justify-content:space-between;font-size:0.75rem;color:#a092d8;margin-bottom:6px;flex-wrap:wrap;gap:4px;">
            ...
            <span>🤖 ${String(item.agent_used).toUpperCase()}</span>
            <span>🌐 ${item.language}</span>
            <span>📅 ${new Date(item.created_at).toLocaleString()}</span>
          </div>
          <div style="font-size:0.85rem;color:#e0d8ff;">⚠️ ${escapeHtml(watchOut.substring(0, 100))}${watchOut.length > 100 ? '…' : ''}</div>
          <div style="font-size:0.7rem;margin-top:4px;color:#b7a5f0;">💰 ${formatUSD(cost)}</div>
        </div>
      `;
    }).join('');
  }
  ...
  async function handlePrediction() {
    ...
    const langName = selectedLang.label.replace(/^\S+\s/, '');
    watchMsg.innerHTML = `🔮 Consulting ${currentAgent.toUpperCase()} oracle in ${langName}… <span class="loading"></span>`;
    expectMsg.innerText = '✨ Gathering cosmic insights…';
    extraMsg.innerText = '⏳ Decoding frequencies…';
    resultLangBadge.style.display = 'none';
    ...
    try {
      const result = await getNumerology(numValue, currentAgent, selectedLang.code);
      ...
      if (selectedLang.code !== 'en') {
        resultLangBadge.innerHTML = `🌐 ${selectedLang.label} reading`;
        resultLangBadge.style.display = 'inline-flex';
      }
      
      // Ensure all cost values are numbers
      const inputTokens = Number(result.input_tokens) || 0;
      const outputTokens = Number(result.output_tokens) || 0;
      const inputCost = Number(result.input_cost) || 0;
      const outputCost = Number(result.output_cost) || 0;
      const totalCost = Number(result.total_cost) || 0;
      
      updateCostUI(
        result.agent || currentAgent,
        inputTokens,
        outputTokens,
        inputCost,
        outputCost,
      ...
      watchMsg.innerText = `⚠️ ${currentAgent.toUpperCase()} error: ${String(err.message).substring(0, 100)}`;
