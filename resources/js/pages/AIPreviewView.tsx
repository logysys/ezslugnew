                                ...
                                <div className="p-6">
                                    {conversation.length > 0 ? (
                                        (() => {
                                            ...
                                            const getCommentsForMessage = (msg: ConversationMessage, index: number) => {
                                                const directComments = effectiveComments.filter(c => c.parent_id === msg.id);
                                                if (index === 0) {
                                                    const orphanComments = effectiveComments.filter(c => !c.parent_id || !convMessageIds.has(c.parent_id));
                                                    return [...directComments, ...orphanComments];
                                                }
                                                ...
                                                <div className="space-y-10">
                                                    {effectiveConversationMessages.map((convMsg, index) => {
                                                        ...
                                                        return (
                                                            <div 
                                                                key={convMsg.id} 
                                                                className={index > 0 ? "pt-10 border-t border-gray-200/80" : ""}
                                                            >
                                                                {/* Conversation Message Row */}
                                                                {renderMessage(convMsg, false)}
                                                                ...
                                                                {commentsForThisMsg.length > 0 && (
                                                                    <div className="mt-6 pt-6 border-t border-gray-100">
                                                                        ...
                                                                        >
                                                                            {commentsForThisMsg.map((comment) => renderMessage(comment, true))}
                                                                        </Masonry>
                                                                    </div>
                                                                )}
                                                            </div>
