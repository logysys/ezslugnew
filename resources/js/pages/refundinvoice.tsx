...
interface InvoiceSummary {
    ...
    buyer_compensation?: number;
    rejection_fee?: number;
...
export default function RefundInvoice() {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            printWindow.document.write(`
                ...
                <html>
                <head>
                    <title>Refund Invoice ${invoice.invoice_number}</title>
                    <style>
                        body { font-family: Arial, sans-serif; margin: 0; padding: 20px; color: #333; }
                        .invoice-header { background: #000; color: #fff; padding: 20px; }
                        ...
                        .penalty { background: #fff3cd; color: #856404; }
                ...
                <body>
                    <div class="invoice-header">
                        <h1>${invoice.is_seller ? 'Penalty' : 'Refund'} Invoice #${invoice.invoice_number}</h1>
                        <p>Issued: ${invoice.issue_date} | Due: ${invoice.due_date}</p>
                        <span class="status ${invoice.status === 'refunded' ? 'refunded' : 'penalty'}">
                    ...
                    <div class="invoice-body">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
                            <div style="background: #f5f5f5; padding: 15px; border-radius: 5px;">
                                <h3 style="font-size: 14px; color: #666; margin: 0 0 10px 0;">Payment Method</h3>
                                <div style="display: flex; align-items: center;">
                                    <span style="font-weight: bold; text-transform: capitalize;">
                                        ${invoice.transaction.payment_method} ending in ${invoice.transaction.last4}
                                    </span>
                                </div>
                            </div>
                            <div style="background: #f5f5f5; padding: 15px; border-radius: 5px;">
                                <h3 style="font-size: 14px; color: #666; margin: 0 0 10px 0;">Transaction ID</h3>
                        ...
                        <div style="margin-bottom: 20px;">
                            <p><strong>Type:</strong> ${invoice.handle.type === 'custom' ? 'Custom Handle' : 'Domain Handle'}</p>
                            <p><strong>Handle:</strong> ${invoice.handle.name}</p>
                            <p><strong>Domain:</strong> ${invoice.handle.domain}</p>
                            <p><strong>URL:</strong> <a href="${invoice.handle.url}" target="_blank">${invoice.handle.url}</a></p>
                        </div>

                        <h2>${invoice.is_seller ? 'Penalty Details' : 'Refund Details'}</h2>
                        <table>
                            <thead>
                                <tr>
                                    <th>Description</th>
                            ...
                            <tbody>
                                ${invoice.items.map(item => `
                                    <tr>
                                        ...
                                        <td>${item.unit_price} EZ$</td>
                                        <td>${item.amount} EZ$</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                            <tfoot>
                        ...
                        ${invoice.summary ? `
                            ...
                            <table class="summary-table">
                                <tbody>
                                    ${Object.entries(invoice.summary).map(([key, value]) => `
                                        <tr>
                                            <th style="text-align: left; padding: 8px; border-bottom: 1px solid #ddd;">${key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</th>
                                            <td style="padding: 8px; border-bottom: 1px solid #ddd; ${value < 0 ? 'color: #dc3545;' : ''}">
                                                ${value < 0 ? '-' : ''}${Math.abs(value).toFixed(2)} EZ$
                                            </td>
                                        </tr>
                                    `).join('')}
                                </tbody>
    ...
    return (
        <>
            <Head>
                <title>{`${invoice.is_seller ? 'Penalty' : 'Refund'} Invoice ${invoice.invoice_number}`}</title>
                <meta name="description" content={`${invoice.is_seller ? 'Penalty' : 'Refund'} invoice for handle transaction - ${invoice.invoice_number}`} />
                {blurStyle}
            </Head>
                ...
                <div className="relative z-10">
                    <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    <h1 className="text-2xl font-bold flex items-center">
                                        ...
                                        {invoice.is_seller ? 'Penalty' : 'Refund'} Invoice #{invoice.invoice_number}
                                    </h1>
                        ...
                        <div className="p-6 md:p-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg">
										...
										<div className="font-mono text-sm">
											{invoice.transaction.refund_reference || invoice.transaction.id}
										</div>
									</div>
                                </div>
                            </div>
                            ...
                            <div className="mb-8">
                                <h2 className="text-lg font-medium text-gray-900 mb-4">Handle Details</h2>
                                <div className="bg-gray-50 p-4 rounded-lg">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <p className="text-sm text-gray-500">Type</p>
                                            <p className="font-medium">
                                                {invoice.handle.type === 'custom' ? 'Custom Handle' : 'Domain Handle'}
                                            </p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-gray-500">Handle</p>
                                            <p className="font-medium">{invoice.handle.name}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-gray-500">Domain</p>
                                            <p className="font-medium">{invoice.handle.domain}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-gray-500">URL</p>
                                            <a 
                                                href={invoice.handle.url} 
                                                target="_blank" 
                            ...
                            <div className="mb-8">
                                <h2 className="text-lg font-medium text-gray-900 mb-4">
                                    {invoice.is_seller ? 'Penalty Details' : 'Refund Details'}
                                </h2>
                                            ...
                                            {invoice.items.map((item, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${
                                                        Number(item.amount) < 0 ? 'text-red-600' : 'text-gray-900'
                                                    }`}>
                                                        {item.amount} EZ$
                                                    </td>
                                                </tr>
                                        ...
                                        <tfoot className="bg-gray-50">
                                            <tr>
                                                ...
                                                <td className={`px-6 py-4 whitespace-nowrap text-sm font-bold ${
                                                    Number(invoice.amount) < 0 ? 'text-red-600' : 'text-gray-900'
                                                }`}>
                                                    {invoice.amount} EZ$
                                                </td>
                                            </tr>
                                                ...
                                                {Object.entries(invoice.summary).map(([key, value]) => (
                                                    <tr key={key}>
                                                        ...
                                                        <td className={`px-6 py-4 whitespace-nowrap text-sm ${
                                                            Number(value) < 0 ? 'text-red-600 font-medium' : 'text-gray-500'
                                                        }`}>
                                                            {Number(value) < 0 ? '-' : ''}{Math.abs(Number(value)).toFixed(2)} EZ$
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
                            ...
                            <div className="flex justify-end space-x-4">
                                ...
                                <Link
                                    href={invoice.is_seller ? "/refundtransferlist" : "/handlesellhistory"}
                                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                                >
                                    Back to {invoice.is_seller ? 'Refund History' : 'Sell History'}
                                </Link>
                            </div>
                        </div>
                    </div>
