...
export default function AISearchView({ 
    ...
    }, [search.conversation_id, applyConversationMessages, canInteract, contentFormat]);
    ...
    }, [search.conversation_id, applyConversationMessages, canInteract]);
    ...
    const handleReplyAskAI = useCallback(async (
        ...
        try {
            ...
            if (data.success) {
                // If it's a comment, it is stored in comments table only - do not alter conversation messages
                if (!data.is_comment && data.conversation_messages && Array.isArray(data.conversation_messages)) {
                ...
                fetchPinnedConversations();
                showAlert(data.is_comment ? 'AI response added to comments!' : 'AI response received!', 'success');
            } else {
    ...
    }, [search.conversation_id, search.thread_id, applyConversationMessages, canInteract, isAiDisabled, maxChars, tooltips, selectedModel]);
    ...
    const renderMessage = (
        ...
        // ============================================================
        // FIX #1: Added 'conversationcomment' to isSpecialContent
        // ============================================================
                ...
                >
                    <div className="flex gap-3">
                        <div className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center shadow-2xs ring-2 ring-white bg-blue-500 text-white">
                            {/* ============================================================ */}
                            {/* FIX #2: Added conversationcomment icon branch                 */}
                            {/* ============================================================ */}
                            {message.content_type === 'upload' ? (
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0 w-full mb-3">
                                {/* ============================================================ */}
                                {/* FIX #3: Added conversationcomment content rendering branch    */}
                                {/* ============================================================ */}
    ...
    const renderDbCommentCard = (c: any) => {
        ...
        return (
            <div 
                key={`db-${c.id}`} 
                className="w-full bg-white rounded-2xl p-4 sm:p-5 border border-gray-200/90 hover:border-gray-300 shadow-2xs hover:shadow-xs transition-all mb-4 group"
            >
                {(userName || userContent || !isAi) && (
                    <>
                        ...
                        {userContent && (
                            <div className="text-sm text-gray-800 leading-relaxed font-normal mb-4 whitespace-pre-wrap break-words">
                                {c.format === 'html' && !isAskAiCard ? (
                                    <div 
                        ...
                        {Array.isArray(c.media) && c.media.length > 0 && (
                            <div className="grid grid-cols-2 gap-2 mb-4">
                                {c.media.map((imgUrl: string, mIdx: number) => (
                                    <img 
                                        key={mIdx} 
                                        src={imgUrl} 
                                        alt="Attachment" 
                                        className="w-full h-28 object-cover rounded-xl border border-gray-200 cursor-pointer hover:opacity-90 transition"
    ...
    const renderCommentsSection = () => {
        ...
        // ============================================================
        // FIX #6: Comment counter includes both 'comment' and 'conversationcomment'
        // ============================================================
        const commentCount = conversation.filter(
            m => m.content_type === 'comment' || m.content_type === 'conversationcomment'
        ).length;
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-medium text-gray-700">Conversation Comments</span>
                    {commentCount > 0 && (
                ...
                <button
                    onClick={() => handleCommentSubmit(undefined, commentContent, 'conversationcomment')}
            ...
            <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-2 mb-3 border-b border-gray-200 pb-3">
                    ...
                    <button
                        ...
                        data-tooltip-id="main-tooltip"
                        data-tooltip-content="Post conversation comments"
                    >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
                        </svg>
