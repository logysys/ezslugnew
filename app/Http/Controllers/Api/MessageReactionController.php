...
class MessageReactionController extends Controller
{
    ...
    public function getReactions($messageId)
    {
        try {
            ...
            // Fetch comments count from dedicated comments table
            try {
                $commentsCount = \App\Models\Comment::where(function($q) use ($messageId, $message) {
                    $q->where('message_id', $messageId);
                    if ($message && $message->slug) {
                        $q->orWhere('message_slug', $message->slug);
                    }
                })->where('status', '!=', 'deleted')->count();
                $reactionData['comments_count'] = $commentsCount;
            } catch (\Exception $e) {
                $reactionData['comments_count'] = 0;
            }
