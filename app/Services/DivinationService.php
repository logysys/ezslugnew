...
class DivinationService
{
    ...
    public function divinate(string $character, string $theme, string $agent): array
    {
...
{\"analysis\":\"對「{$character}」拆字解析，分析部首、筆畫、字形結構及字義，約100字\",\"luck\":\"針對「{$theme}」主題，給出正面指引與吉象，約80字\",\"caution\":\"針對「{$theme}」主題，給出需留意或避免的事項，約80字\",\"tip\":\"一句精簡有力的今日錦言，30字以內\"}";
        ...
        $payload = match ($agent) {
            ...
            'perplexity' => [
                ...
                'messages' => [
                    ...
                    ['role' => 'user', 'content' => $prompt . ' 只回傳JSON，不含任何其他文字。']
                ],
                'stream' => false,
                'temperature' => 0.7,
                'max_tokens' => 2000
    ...
    protected function cleanAndParseJSON(string $rawText): array
    {
        ...
        try {
            ...
            // If we got at least some fields, return them
            if (!empty($result)) {
                return $result;
            }

            throw new \Exception('JSON 解析失敗: ' . substr($cleaned, 0, 200));
        }
    }
