...
class NumerologyOracleService
{
    ...
    protected function makeRequest(string $agent, array $provider, string $systemPrompt, int $number)
    {
        ...
        Log::info("Calling numerology API", ['agent' => $agent, 'number' => $number, 'url' => $url]);
    ...
    protected function parseResponse(string $agent, array $response): array
    {
        ...
        return $this->parseUnifiedResponse($agent, $response);
    }
    ...
    protected function parseUnifiedResponse(string $agent, array $response): array
    {
