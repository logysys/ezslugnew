...
class NumerologyOracleTwoService
{
    ...
    {
        $langName = $this->languages[$targetLanguage] ?? $targetLanguage;
        $provider = $this->config['kimi'];

        $prompt = "Translate the following text into {$langName}. Keep the poetic, mystical tone. Return ONLY the translated text, no explanations or additional commentary:\n\n{$text}";

        $response = Http::withHeaders([
            'Content-Type' => 'application/json',
            'Authorization' => 'Bearer ' . $provider['key']
        ])->timeout(30)->post($provider['url'], [
            'model' => $provider['model'],
            'messages' => [
                ['role' => 'system', 'content' => 'You are a professional translator. Always respond with the translated text only.'],
        ...
        if (!$response->successful()) {
            ...
            throw new \Exception("Translation API error: {$error}");
        }
        ...
        if (empty($translated)) {
            throw new \Exception('Translation returned empty response');
        }
    ...
    {
        return $this->pricing[$agent] ?? ['input' => 1, 'output' => 2, 'label' => 'Unknown'];
    }
